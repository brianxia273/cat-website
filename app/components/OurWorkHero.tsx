      ...
      <div className="hidden lg:flex items-center">
        <div className="h-80 w-106">
          <Image src={HeroPic} alt="Cornell Assistive Technologies team member working on the sensory watch project" height={HeroPic.height} width={HeroPic.width}
            className={`rounded-[20px] drop-shadow-[0_4px_4px_rgba(0,0,0,0.5)] origin-right scale-60 lg:scale-85 xl:scale-110 2xl:scale-130 ${imgLoadStyles}`}
            onLoadingComplete={(img) => { loadingComplete(img) }} />
        </div>
      </div>
      {/* mobile version */}
      <div className="flex lg:hidden items-end justify-center">
        <div className="h-80 sm:h-86 w-106 -mb-5 sm:mb-0 origin-bottom scale-90 sm:scale-110 md:scale-120 lg:scale-75 xl:scale-100">
          <Image src={HeroPic} alt="Cornell Assistive Technologies team member working on the sensory watch project" height={HeroPic.height} width={HeroPic.width}
...
const HeroContainer = ({ children }: any) => {
  return (
    <div className="flex flex-col">
      <div className="h-145 sm:h-200 md:h-220 lg:h-155 xl:h-170 2xl:h-190 mt-10 universepad flex flex-col-reverse justify-start lg:justify-between lg:flex-row gap-12 relative overflow-hidden lg:items-center">
